...
// "gid" es OPCIONAL: el número de pestaña dentro de esa hoja de cálculo (se ve en la URL de
// Google Sheets como ".../edit?gid=NÚMERO#gid=NÚMERO"). Sin "gid", se exporta la primera
// pestaña — que no siempre es la que tiene las respuestas vigentes si la hoja tiene varias.
...
// "pre" | "pos" | null — de qué lado de una capacitación con evaluación antes/después viene
// esta hoja (p. ej. "PRE Radiadores Planta de Oxígeno" vs "POS Radiadores Planta de
// Oxígeno", configuradas con el mismo `label` para que el dashboard las trate como un solo
// tema). Se puede fijar explícito con "fase" en CAPACITACIONES_SHEETS; si no está, se infiere
// del "id" configurado (por convención termina en "_pre"/"_pos").
function trainingFase(training) {
  if (training.fase === 'pre' || training.fase === 'pos') return training.fase;
  const id = (training.id || '').toLowerCase();
  if (id.endsWith('_pre') || id.startsWith('pre_')) return 'pre';
  if (id.endsWith('_pos') || id.startsWith('pos_')) return 'pos';
  return null;
}
...
// `gid` es opcional: sin él, Google exporta la PRIMERA pestaña de la hoja de cálculo — que
// no siempre es la pestaña con las respuestas vigentes (algunas hojas de este proyecto
// tienen varias pestañas, p. ej. una vieja y una donde Google Forms sigue escribiendo las
// respuestas actuales). Cuando eso pasa, hay que fijar el `gid` exacto de esa pestaña en
// CAPACITACIONES_SHEETS (se ve en la URL de Google Sheets: ".../edit?gid=NÚMERO#gid=NÚMERO").
async function fetchSheetCsv(sheetId, gid, retries = 2) {
...
function parseTrainingSheet(training, csvText) {
  ...
  const fase = trainingFase(training);
...
export async function buildCapacitacionesSnapshot() {
  ...
  for (let i = 0; i < trainings.length; i += CAPACITACIONES_CONCURRENCIA) {
    ...
    await Promise.all(lote.map(async (training) => {
      try {
        const csv = await fetchSheetCsv(training.sheetId, training.gid);
