...
// serverless que consulta ~18 hojas de golpe. Un reintento breve resuelve la mayoría de esos
// casos transitorios sin que el usuario tenga que presionar "Actualizar" de nuevo.
async function fetchSheetCsv(sheetId, retries = 2) {
  const url = `https://docs.google.com/spreadsheets/d/${sheetId}/export?format=csv`;
  for (let intento = 0; ; intento++) {
    const res = await fetch(url);
    if (res.ok) return res.text();
    if (intento >= retries) throw new Error(`HTTP ${res.status}`);
