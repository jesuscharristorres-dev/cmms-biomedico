...
async function loadEquipos() {
  try {
    const res = await window.storage.get('cmms-equipos', false);
    if (res && res.value) return JSON.parse(res.value);
  } catch (e) { /* sin datos aún */ }
  return [];
}
async function saveEquipos(equipos) {
  try { await window.storage.set('cmms-equipos', JSON.stringify(equipos), false); }
  ...
  return (
    <div className={`flex h-full min-h-[700px] font-sans ${t.bg} ${t.text}`} style={{ fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {menu === 'configuracion' && <ConfigPage t={t} accent={accent} onReset={() => { if (confirm('¿Borrar todos los equipos guardados?')) setEquipos([]); }} />}
      </div>
...
/* ---------------------------------------------------------------- */
function ConfigPage({ t, accent, onReset }) {
  return (
    <div>
