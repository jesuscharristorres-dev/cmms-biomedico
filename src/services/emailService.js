...
/** 3. Se dispara cuando un preventivo queda próximo a vencer o ya venció. */
export async function notifyPreventivoProximo(equipo, alerta, destinatarios) {
  const vencido = alerta.status === 'vencido';
  const html = baseTemplate({
    colorFranja: '#F59E0B',
    eyebrow: vencido ? 'Preventivo vencido' : 'Preventivo próximo a vencer',
    titulo: equipo.equipo || equipo.equipoNombre || 'Equipo sin nombre',
    filas: [
      ['Empresa', equipo.empresa],
      ['Sede', equipo.sede],
      ['Fecha programada', alerta.fecha],
      ['Estado', vencido ? `Vencido hace ${Math.abs(alerta.diffDays)} días` : `Faltan ${alerta.diffDays} días`],
    ],
  });
  return sendEmail({
    to: destinatarios,
    subject: `📅 Preventivo ${vencido ? 'vencido' : 'próximo a vencer'} — ${equipo.equipo || 'Equipo'}`,
    html,
  });
}

/** 4. Se dispara cuando una calibración queda próxima a vencer o ya venció. */
export async function notifyCalibracionProxima(equipo, alerta, destinatarios) {
  const vencido = alerta.status === 'vencido';
  const html = baseTemplate({
    colorFranja: '#0EA5E9',
    eyebrow: vencido ? 'Calibración vencida' : 'Calibración próxima a vencer',
    titulo: equipo.equipo || equipo.equipoNombre || 'Equipo sin nombre',
    filas: [
      ['Empresa', equipo.empresa],
      ['Sede', equipo.sede],
      ['Última calibración', equipo.fechaUltimaCalibracion],
      ['Estado', vencido ? `Vencida hace ${Math.abs(alerta.diffDays)} días` : `Faltan ${alerta.diffDays} días`],
    ],
  });
  return sendEmail({
    to: destinatarios,
    subject: `🧪 Calibración ${vencido ? 'vencida' : 'próxima a vencer'} — ${equipo.equipo || 'Equipo'}`,
    html,
  });
}

/** Resumen manual de todas las alertas activas (botón "Enviar alertas por correo"). */
export async function sendAlertsSummary(alerts, destinatarios) {
  const filas = alerts.map((a) => [
    `${a.tipo} — ${a.equipo}`,
    `${a.empresa} · ${a.sede} · ${a.status === 'vencido' ? `vencida hace ${Math.abs(a.diffDays)} días` : `faltan ${a.diffDays} días`}`,
  ]);
  const html = baseTemplate({
    colorFranja: '#4FD1C5',
    eyebrow: `${alerts.length} alerta${alerts.length !== 1 ? 's' : ''} activa${alerts.length !== 1 ? 's' : ''}`,
    titulo: 'Resumen de alertas de mantenimiento',
    filas,
  });
  return sendEmail({
    to: destinatarios,
    subject: `📋 Resumen de alertas — ${alerts.length} pendiente${alerts.length !== 1 ? 's' : ''}`,
    html,
  });
}

/* ---------------------------------------------------------------- */
/* DEDUPLICACIÓN — evita reenviar la misma alerta en cada visita     */
/* ---------------------------------------------------------------- */
// Como esta app no tiene un backend con cron, el chequeo de "próximos a vencer"
// ocurre cada vez que alguien abre el CMMS. Para no reenviar el mismo correo una
// y otra vez, se guarda localmente qué alertas ya se notificaron.

const NOTIFIED_KEY = 'cmms-notified-alerts';
...
export function markNotified(ids) {
  ...
  try {
    ...
    console.error('[emailService] No se pudo guardar el registro de notificados', e);
  }
}
